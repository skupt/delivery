...
import rozaryonov.delivery.dao.impl.LogisticNetElementDao;
import rozaryonov.delivery.entities.LogisticNetElement;

public class PathFinder {
	private Set<Town> townSet;
	
	public PathFinder(Connection cn, long netConfigId) {
		townSet=loadTowns(cn, netConfigId);
	}
	...
	private static Set<Town> loadTowns(Connection connection, long netConfigId) {
		LogisticNetElementDao edao = new LogisticNetElementDao(connection);
		Iterable<LogisticNetElement> eList = edao.findByNetConfig(netConfigId);
	...
	public static void main(String[] args) throws ClassNotFoundException, IOException, SQLException {
		...
		String start = "Львов";
		String finish = "Запорожье";
		Connection con = ConnectionWrapper.getConnection();
		System.out.println(shortestPath(con, 1, start, finish));
		System.out.println(minDistance(con, 1, start, finish));

		long startId = 2;
		long finishId = 16;
		System.out.println(shortestPath(con, 1, startId, finishId));
		System.out.println(minDistance(con, 1, startId, finishId));
